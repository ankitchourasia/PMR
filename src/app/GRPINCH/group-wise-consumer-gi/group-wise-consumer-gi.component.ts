...
@Component({
  selector: 'app-group-wise-consumer-gi',
  templateUrl: './group-wise-consumer-gi.component.html',
  styleUrls: ['./group-wise-consumer-gi.component.css']
})
export class GroupWiseConsumerGIComponent implements OnInit {
  ...
  getGroupWiseConsumer()
  {
    ...
    let formdata : FormData = new FormData();
      ...
      return this.http.post("api/consumer-reports/get-group-wise-consumer-in-groups", formdata, {headers:new HttpHeaders().set('Authorization',this.session.get('token'))}).subscribe(response=>
      {
        this.resp=response;
        if(this.resp!=null)
    {
