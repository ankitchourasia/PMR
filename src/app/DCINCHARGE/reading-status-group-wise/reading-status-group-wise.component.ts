...
@Component({
  selector: 'app-reading-status-group-wise',
  templateUrl: './reading-status-group-wise.component.html',
  styleUrls: ['./reading-status-group-wise.component.css']
})
export class ReadingStatusGroupWiseComponent implements OnInit {
...
billMonths = GlobalConstants.billMonths;
  ...
  getReadingStatus()
  {
    ...
    return this.http.post("api/dc-incharge-reports/group-wise-count-dashboard", formdata, {headers:new HttpHeaders().set('Authorization',this.session.get('token'))}).subscribe(response=>
    {
      this.resp=response;
      if(this.resp!='')
  {
