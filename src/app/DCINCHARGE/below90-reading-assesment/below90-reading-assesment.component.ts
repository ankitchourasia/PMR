...
declare var $ :any;
@Component({
  selector: 'app-below90-reading-assesment',
  templateUrl: './below90-reading-assesment.component.html',
  styleUrls: ['./below90-reading-assesment.component.css']
})
export class Below90ReadingAssesmentComponent implements OnInit {
...
VerifyClick(custid,imageid,srno)
{
  ...
  { 
    ...
    console.log( this.SelectedList+" "+this.SelectedImageId);
  ...
  return this.http.post("api/assesment-records/update-reading-to-approve", formdata, {headers:new HttpHeaders().set('Authorization',this.session.get('token'))}).subscribe(response=>
  {
    ...
    if(this.globalCounter<this.SelectedList.length-1)
    {
      this.globalCounter=this.globalCounter+1;
        alert("Global Counter "+this.globalCounter + " "+this.SelectedList.length);
        this.approveMeter(this.globalCounter);
...
editAssesment()
  ...
  exportAsXsls(): void {
    this.exportService.exportAsExcelFile(this.resp, 'below 90'+this.groupno+"_"+this.billmonth);
