...
@Component({
  selector: 'app-dcinch-smart-meter-report',
  templateUrl: './dcinch-smart-meter-report.component.html',
  styleUrls: ['./dcinch-smart-meter-report.component.css']
})
export class DcinchSmartMeterReportComponent implements OnInit {
  ...
  getByGroupNoAndBillMonth(groupNo){
    ...
    return this.http.post("api/spot-bill-reporter/get-list-for-group", formdata, { headers: new HttpHeaders().set('Authorization', this.session.get('token')) }).subscribe(response => {
      this.loading = false;
      console.log(response);
  ...
  exportAsXsls(): void {
    this.exportService.exportAsExcelFile(this.consumers, "smart_meter_consumers_" + this.billMonth);
  }
