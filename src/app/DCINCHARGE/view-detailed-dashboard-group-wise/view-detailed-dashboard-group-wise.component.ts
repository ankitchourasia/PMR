...
@Component({
  selector: 'app-view-detailed-dashboard-group-wise',
  templateUrl: './view-detailed-dashboard-group-wise.component.html',
  styleUrls: ['./view-detailed-dashboard-group-wise.component.css']
})
export class ViewDetailedDashboardGroupWiseComponent implements OnInit {
  ...
  ngOnInit() 
  {
    ...
    this.groupno=this.activeroute.snapshot.params.grno;
  ...
  getReadingsDatewiseX()
    {
      ...
      return this.http.post("api/dc-incharge-reports/get-list-dashboard-list-groupwise", formdata, {headers:new HttpHeaders().set('Authorization',this.session.get('token'))}).subscribe(response=>
      {
        this.resp2=response;
