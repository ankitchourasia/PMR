...
@Component({
  selector: 'app-rejected-readings',
  templateUrl: './rejected-readings.component.html',
  styleUrls: ['./rejected-readings.component.css']
})
export class RejectedReadingsComponent implements OnInit {
  ...
  error=>{
   this.error = error
    ...
    return this.http.post("api/reading-operations/get-rejected-readings", formdata, {headers:new HttpHeaders().set('Authorization',this.session.get('token'))}).subscribe(response=>
    {
      this.resp2=response;
      if(this.resp2!='')
      {
