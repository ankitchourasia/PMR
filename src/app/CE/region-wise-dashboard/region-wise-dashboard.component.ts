...
@Component({
  ...
  templateUrl: './region-wise-dashboard.component.html',
  styleUrls: ['./region-wise-dashboard.component.css']
})
export class RegionWiseDashboardComponent implements OnInit {
  ...
  ngOnInit() 
  {
    if(this.session.get('check')=="FOUNDOK" && this.session.get('role')=="CEUSER")
    {
      // alert(this.division);
    }
    else  
  ...
  getRegionDashboard()
  {
    ...
    return this.http.post("api/ce-dashboard/get-ce-dashboard-full", formdata, {headers:new HttpHeaders().set('Authorization',this.session.get('token'))}).subscribe(response=>
    {
      this.resp=response;
      if(this.resp!='')
      {
  ...
  getCircleDashboard()
  {
    ...
    return this.http.post("api/ce-dashboard/get-circle-wise-deatils-for-ce", formdata, {headers:new HttpHeaders().set('Authorization',this.session.get('token'))}).subscribe(response=>
    {
      this.resp2=response;
      if(this.resp2!='')
      {
